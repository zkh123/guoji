...
import com.sdkbackendpr.config.userinfo.DBUserInfoShardAlgo;
import com.sdkbackendpr.config.userinfo.TblRangeShardAlgo;
import com.sdkbackendpr.config.userinfo.TblUserInfoShardAlgo;
...
public class DataSourceConfig {
    ...
    public DataSource shardingDataSource() throws SQLException {
        ...
        // 默认的分库策略
        src.setDefaultDatabaseShardingStrategyConfig(new StandardShardingStrategyConfiguration("user_id", DBUserInfoShardAlgo.class.getName()));
        // 默认的分表策略
        src.setDefaultTableShardingStrategyConfig(new StandardShardingStrategyConfiguration("user_id", TblUserInfoShardAlgo.class.getName(),TblRangeShardAlgo.class.getName()));
        ...
        TableRuleConfiguration tableRuleConfiguration4 = getSdkDataLogTableRuleConfiguration();
        list.add(tableRuleConfiguration1);list.add(tableRuleConfiguration2);list.add(tableRuleConfiguration3);list.add(tableRuleConfiguration4);
...
//        result.setDriverClassName("com.mysql.cj.jdbc.Driver");
    ...
    public TableRuleConfiguration getSdkDataLogTableRuleConfiguration() {
        TableRuleConfiguration userTableRuleConfig=new TableRuleConfiguration();
        userTableRuleConfig.setLogicTable("tb_sdk_data_log");
        userTableRuleConfig.setDatabaseShardingStrategyConfig(new StandardShardingStrategyConfiguration("user_id", DBDefaultShardAlgo.class.getName()));
        userTableRuleConfig.setTableShardingStrategyConfig(new StandardShardingStrategyConfiguration("user_id",TblDefaultShardAlgo.class.getName(), TblRangeShardAlgo.class.getName()));
        return userTableRuleConfig;
    }
